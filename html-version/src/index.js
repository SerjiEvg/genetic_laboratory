...
var uiListeners = {
  ...
  carDeath(carInfo){
    ...
    // После гибели автомобиля возвращаем камеру к лидеру
    ...
    if (leaderPosition.leader == k) {
      // Лидер погиб — ищем нового лидера
      cw_findLeader();
    }
  },
...
function cw_confirmResetWorld() {
  if (confirm('Действительно создать новый мир?')) {
    cw_resetWorld();
  } else {
    return false;
  }
...
// Работа с повтором лучшего автомобиля
...
function cw_toggleGhostReplay(button) {
  if (cw_ghostReplayInterval == null) {
    ...
    cw_stopGhostReplay();
    button.value = "Посмотреть лучший повтор";
  }
}
// Работа с повтором лучшего автомобиля END

// Начальная настройка, выполняется один раз
function cw_init() {
  // Клонируем маркер и индикатор состояния автомобиля
  ...
  for (var k = 0; k < generationSize; k++) {
    ...
    // Индикаторы состояния автомобилей
...
minimapholder.onclick = function (event) {
  ...
  if (closest.x == maxX) { // снова фокусируемся на лидере
    cw_setCameraTarget(-1);
  } else {
...
function cw_setGravity(choice) {
  ...
  // ПРОВЕРЯЕМ ИЗМЕНЕНИЕ ГРАВИТАЦИИ

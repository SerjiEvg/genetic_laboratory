...
function visChart(elem, scores, propertyMap, graph) {
  ...
  // Создаём и заполняем таблицу данных.
  ...
  scores.forEach(function(scoreInfo){
    data.add({
      ...
      // дополнительные данные: происхождение
    });
  });
  ...
  var options = {
    ...
    tooltip: function (point) {
      // точка содержит координаты x, y, z и исходные данные
      // data — исходный объект, переданный при создании точки
  ...
  if (camera) graph.setCameraPosition(camera); // Восстанавливаем положение камеры
  return graph;
}
